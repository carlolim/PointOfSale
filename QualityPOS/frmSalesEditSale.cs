...
{
    public partial class frmSalesEditSale : Form
    {
        ...
        public SaleDTO Transaction { get; set; }
        ...
        private async void LoadTransaction()
        {
            ...
            var salesProducts = await _saleManager.GetSaleProductsBySaleID(SaleID);
            foreach(var p in salesProducts)
            {
                var total = Convert.ToDecimal(p.Quantity) * p.SalesPrice;
                var i = dataGridView1.Rows.Add(p.ProductID, p.ProductName, p.Quantity, p.SalesPrice, total, "Remove");
        ...
        private void txtQuantity_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (_selected != null)
                {
                    var total = Convert.ToDecimal(txtQuantity.Text) * _selected.SalesPrice;
        ...
        {
            decimal decimalOut;
            if (!decimal.TryParse(dataGridView1.Rows[e.RowIndex].Cells["colQuantity"].Value.ToString(), out decimalOut))
            {
                dataGridView1.Rows[e.RowIndex].Cells["colQuantity"].Value = 1;
                return;
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if(dataGridView1.RowCount > 0)
            {
                using(frmSalesPayment frm = new frmSalesPayment())
                {
                    ...
                    frm.Total = subtotal;
                    frm.AmountPaid = Convert.ToDecimal(Transaction.AmountPaid);
                    frm.SaleProducts = saleProducts;
                    frm.UpdateTransaction = true;
                    frm.SaleID = SaleID;
                    frm.ShowDialog();
